...
export const authUidContext = createContext("");
...
const Header = ({ children }: {children: ReactNode}) => {
  ...
  return (
    <div className='w-screen bg-white shadow-md'>
      ...
      {/* title */}
      <div className='fixed flex items-center justify-between shadow-md bg-white w-[100vw] px-4 pt-2 pb-2 z-100'>
        <div>
        ...
        {/* ＜未実装部分 1＞
          <div className='absolute z-100 top-8 right-8 w-10 h-4 flex flex-col justify-between' onClick={menuState}>
      ...
      {/* menu contents　＜未実装部分 1＞
      <div className={`absolute top-[-100vh] z-20 h-screen w-screen bg-white flex items-center transition ease-in-out delay-800 duration-300 ${menu ? 'opacity-100 translate-y-[100vh]' : 'opacity-40'}`}>
      ...
      {/* content */}
      <authUidContext.Provider value={userUid}>
        { children }
      </authUidContext.Provider>

      {/* bottom contents */}
      <div className='fixed z-0 bottom-0'>
        <div className='w-screen h-16 pt-1 bg-[#F1DAAE]'>
          <ul className='flex justify-around mt-2 text-center text-xs font-semibold text-[#412312]'>
            <li className='w-[16vw]'>
